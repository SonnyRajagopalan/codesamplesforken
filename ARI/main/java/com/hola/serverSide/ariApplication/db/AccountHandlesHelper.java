...
import com.hola.serverSide.ariApplication.db.beans.HandleStatus;
...
public class AccountHandlesHelper
{
    ...
    public static String getPJSIPpedAppHandleForFirstVoiceTech (String usernameOrEmail)
    {
	AccountHandles accountHandles = getAccountHandlesByUsernameOrEmail (usernameOrEmail);
	String firstVoiceHandleAvailable="NO_VOICE_HANDLE_AVAILABLE";
	/*
	  [{"app":"voip","method":"voice","handle":"","status":true},
	   {"app":"mobile","method":"text","handle":"+13392061413","status":false},
	   {"app":"mobile","method":"voice","handle":"+13392061413","status":false}]
	...
	try
		...
		List <HandleStatus>
		    handleStatusesJsonList = objectMapper.readValue (accountHandles.getHandles (),
							       new TypeReference<List <HandleStatus>> () {});
		String pjsippifiedMobileHandle=null, pjsippifiedLandlineHandle=null, pjsippifiedVoipHandle=null;
		
		for (HandleStatus handleStatus: handleStatusesJsonList)
		    {
			if (handleStatus.getApp ().equals ("voip") && handleStatus.getMethod ().equals ("voice") && 
			    handleStatus.getStatus () != false)
			    {
				pjsippifiedVoipHandle = "PJSIP/" + handleStatus.getHandle ();
			    }

			if (handleStatus.getApp ().equals ("mobile") && handleStatus.getMethod ().equals ("voice") && 
			    handleStatus.getStatus () != false)

			    {
				pjsippifiedMobileHandle = "PJSIP/" + handleStatus.getHandle () + "@twilio-siptrunk";
			    }

			if (handleStatus.getApp ().equals ("landline") && handleStatus.getMethod ().equals ("voice") && 
			    handleStatus.getStatus () != false)
			    {
				pjsippifiedLandlineHandle = "PJSIP/" + handleStatus.getHandle () + "@twilio-siptrunk";
			    }
			else
			    {
				// Do nothing, need to check the next
			    }
		    }
		if ((pjsippifiedVoipHandle == null) && (pjsippifiedMobileHandle == null) & (pjsippifiedLandlineHandle == null))
		    {
			return null;
		    }
		else if (pjsippifiedVoipHandle != null)
		    {
			return pjsippifiedVoipHandle;
		    }
		else if (pjsippifiedMobileHandle != null)
		    {
			return pjsippifiedMobileHandle;
		    }
		else if (pjsippifiedLandlineHandle != null)
		    {
			return pjsippifiedLandlineHandle;
		    }
	    }
	catch (IOException e)
	    {
	...
	for (AccountHandles accountHandles: allAccountHandles)
	    {
		//System.out.println (accountHandles.toJsonString ());
